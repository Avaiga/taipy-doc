...
as a best practice page for managing data nodes in a Taipy application by leveraging Taipy visual
elements.

Out-of-the-box, the data management page allows the user to perform various operations on the data
nodes, such as filtering, sorting, searching for data nodes, selecting data nodes, updating the
data, and visualizing each data node's content.

<figure>
  <img src="../img/dmp_template_dark.jpeg" class="visible-dark" />
  <img src="../img/dmp_template_light.jpeg" class="visible-light"/>
  <figcaption>Taipy data node management page created by the data node management page template
...
- **Comprehensive Features**: The template provides a best-practice page scaffold for a
    data node management page, which supports a wide range of functionalities, providing a
...
[1/3] Page title (data_node_management_page):
...
## 3. Binding variable for "selected_data_node"
...
- If there is already a binding variable with the same purpose from a different page, please
    ...
    to the data node management page.
...
You can also explicitly import the binding variable `selected_data_node`  from the newly created
page to a different page to share the data node selected by the user.
