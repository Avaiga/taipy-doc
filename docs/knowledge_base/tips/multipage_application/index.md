...
We've prepared a two-part series of articles to explain how to use Taipy's multi-page 
functionality:

In Part 1, we'll go over the basics of creating a multi-page application, including 
...
So, if you're ready to enhance your data visualization, check out our multi-page feature 
...
(Taipy [Markdown](https://docs.taipy.io/en/latest/manuals/reference/taipy.gui.Markdown/)
or [HTML](https://docs.taipy.io/en/latest/manuals/reference/taipy.gui.Html/) objects) 
to the `taipy.gui.Gui` constructor. For example, to turn `home.py` into a one-page Taipy 
application, we could add these lines:
...
documentation, then call `navigate(state, page)` to direct the user to the selected `page`. 
In practice, we effectively have the same functionality as the navbar, but had to do a bit more 
