...
-   :octicons-feed-rocket-16:{ .rocket-icon title="Improvement"} The _unselected_value_
    ...
    [_value_](../../refmans/gui/viselements/generic/toggle.md#p-value) property set to None.
...
-   :octicons-feed-plus-16:{ .plus-icon title="New feature" } The `CSVDataNode^`
    now supports the _separator_ parameter to specify the separator used in the CSV file.<br/>
...
-   :octicons-feed-rocket-16:{ .rocket-icon title="Improvement"} `SQLDataNode^` now
    ...
    parameter: either _read_query_ for read-only or _write_query_builder_ for write-only.
...
-   :octicons-bug-24:{ .bug-icon title="Bug fix" } Taipy now only checks the compatibility
    of the installed _taipy-core_ version with the versions set in the existing entities
