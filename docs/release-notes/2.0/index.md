...
- :octicons-bug-24:{ .bug-icon title="Bug fix" } `image` control may not render properly.<br/>
  See [issue #436](https://github.com/Avaiga/taipy-gui/issues/436).
- :octicons-bug-24:{ .bug-icon title="Bug fix" } Clarify and improve the `editable` (and `editable[]`) property
  in the `table` control.<br/>
  See [issue #464](https://github.com/Avaiga/taipy-gui/issues/464).
- :octicons-bug-24:{ .bug-icon title="Bug fix" } [gui] section in configuration files breaks the application.<br/>
...
- :octicons-bug-24:{ .bug-icon title="Bug fix" } Bar charts' "barmode" set to "stack" is broken.<br/>
...
- :octicons-feed-plus-16:{ .plus-icon title="New feature" } Extension API: custom visual elements can be integrated
...
- :octicons-feed-plus-16:{ .plus-icon title="New feature" } New callbacks (`on_init`, `on_navigate`, `on_exception`
...
- :octicons-feed-plus-16:{ .plus-icon title="New feature" } New functions allow applications to invoke long-running
  callbacks without blocking.<br/>
  See [Long Running Callbacks](../../userman/gui/callbacks.md#long-running-callbacks) for
  details.
- :octicons-feed-plus-16:{ .plus-icon title="New feature" } The Taipy GUI application configuration uses the generic
  Taipy configuration mechanism exposed in the new `taipy-config` package.
- :octicons-feed-plus-16:{ .plus-icon title="New feature" } An application can request the status of the server
  application using the "status" predefined page.<br/>
- :octicons-feed-plus-16:{ .plus-icon title="New feature" } The new 'base' property of the chart control makes it
...
- :octicons-feed-rocket-16:{ .rocket-icon title="Improvement"} Stopping then re-running the `Gui^` instance is no
  longer required in Notebook contexts.
- :octicons-feed-rocket-16:{ .rocket-icon title="Improvement"} A discrete graphical indicator is displayed at the
...
- :octicons-bug-24:{ .bug-icon title="Bug fix" } Do not update `last_edit_date` when a job fails or is abandoned.
...
- :octicons-feed-plus-16:{ .plus-icon title="New feature" } New data node named SQLTableDataNode. It represents a
  table in a SQL database.
- :octicons-feed-plus-16:{ .plus-icon title="New feature" } New data node named JSONDataNode. It represents the data
  from a JSON file.
- :octicons-feed-plus-16:{ .plus-icon title="New feature" } SQLDataNode behavior is changed due to the release of
  `SQLTableDataNode`. Now it represents the data using custom read and write queries.
- :octicons-feed-plus-16:{ .plus-icon title="New feature" } In standalone mode, a job whose status is `SUBMITTED`,
...
- :octicons-feed-plus-16:{ .plus-icon title="New feature" } Taipy Orchestrator can now be run as a service by using
...
- :octicons-feed-rocket-16:{ .rocket-icon title="Improvement"} The data node of a scenario or a pipeline can now be
  accessed directly at the scenario or pipeline
  levels.
- :octicons-feed-rocket-16:{ .rocket-icon title="Improvement"} When submitting a scenario, a pipeline, or a task, the
  job(s) created will be returned.
- :octicons-feed-rocket-16:{ .rocket-icon title="Improvement"} When submitting a scenario, pipeline, or task in
...
- :octicons-feed-rocket-16:{ .rocket-icon title="Improvement"} When in standalone mode, the job dispatcher runs in a
  sub-thread that periodically checks for new jobs submitted by Taipy to execute.
- :octicons-feed-rocket-16:{ .rocket-icon title="Improvement"} When a running job fails, its subsequent jobs will be
  automatically abandoned.
- :octicons-feed-rocket-16:{ .rocket-icon title="Improvement"} A primary scenario can be deleted along with its cycle
  if it is the only scenario in the cycle.
- :octicons-feed-rocket-16:{ .rocket-icon title="Improvement"} The messages of the various Exceptions that can be
  raised have been improved to help the users debug their applications.

<h4>Deprecations</h4>

- :octicons-alert-fill-24:{ .alert-icon title="Deprecation" } The field *nb_of_workers* within the Config has been
...
- :octicons-feed-plus-16:{ .plus-icon title="New feature" } The new `taipy-config` package was exposed to be used by any
