...
- :octicons-feed-plus-16:{ .plus-icon } Taipy application can now be run with the
...
- :octicons-feed-plus-16:{ .plus-icon } A new package holds the
...
- :octicons-feed-plus-16:{ .plus-icon } You can now update variables on all clients
  using the *shared variables* concept. See the `Gui.add_shared_variable()^` and
  `State.dispatch()^` methods for details.
- :octicons-feed-plus-16:{ .plus-icon } You can now invoke a callback for all clients
...
- :octicons-feed-plus-16:{ .plus-icon } The
  ...
  for more information.
- :octicons-feed-plus-16:{ .plus-icon } A new CSS class called *toggle-navbar* was added to the
...
- :octicons-feed-plus-16:{ .plus-icon } The [`chart`](../../refmans/gui/viselements/generic/chart.md)
...
- :octicons-alert-fill-24:{ .alert-icon } The *action* parameter of the `on_action` callback
...
- :octicons-feed-rocket-16:{ .rocket-icon } The `navigate()^` function has an additional
...
- :octicons-feed-rocket-16:{ .rocket-icon } The *on_action* parameter of the `download()^`
  function can be a function and not just a function name.
- :octicons-feed-rocket-16:{ .rocket-icon } Setting the *debug* parameter of `Gui.run()^`
  to True provides stack traces to be shown in the console when exceptions occur in user code.
- :octicons-bug-24:{ .bug-icon } The callback function set to the *on_action* parameter
...
- :octicons-bug-24:{ .bug-icon } Setting the
...
- :octicons-bug-24:{ .bug-icon } Variables imported by an `import *` directive are not
...
- :octicons-bug-24:{ .bug-icon } The [`date`](../../refmans/gui/viselements/generic/date.md)
...
- :octicons-bug-24:{ .bug-icon } Updating a
...
- :octicons-bug-24:{ .bug-icon } Selection in a
...
- :octicons-bug-24:{ .bug-icon } Hover text doesn't show properly in a
  [`selector`](../../refmans/gui/viselements/generic/selector.md) that
  is crowded.<br/>
  See [issue #927](https://github.com/Avaiga/taipy-gui/issues/927).
- :octicons-bug-24:{ .bug-icon } Options with a long text in a
...
- :octicons-bug-24:{ .bug-icon } The [`table`](../../refmans/gui/viselements/generic/toggle.md)
...
- :octicons-feed-plus-16:{ .plus-icon } A production version of a Taipy application can now
...
- :octicons-feed-plus-16:{ .plus-icon } A `GLOBAL` scope data node can be created from a
...
- :octicons-feed-plus-16:{ .plus-icon } A data node configuration can be built from an
...
- :octicons-feed-plus-16:{ .plus-icon } A new class `Submittable^` models entities that
...
- :octicons-feed-plus-16:{ .plus-icon } New functions exposed by the `taipy` module:
    * `taipy.is_deletable()^` checks if an entity can be deleted;
    * `taipy.exists()^` checks if an entity exists.
- :octicons-feed-plus-16:{ .plus-icon } The encoding type of CSVDataNode and JSONDataNode
  ...
  sections.
- :octicons-alert-fill-24:{ .alert-icon } A `ScenarioConfig^` graph is now created directly
...
- :octicons-alert-fill-24:{ .alert-icon } The `Pipeline` object has been removed and replaced
...
- :octicons-alert-fill-24:{ .alert-icon } `Scope.PIPELINE` has been removed from possible `Scope^`
  values.
- :octicons-alert-fill-24:{ .alert-icon } The `root_folder`, `storage_folder`, `read_entity_retry`,
  ...
  details.
- :octicons-alert-fill-24:{ .alert-icon } The `clean_entities` attribute has been removed from
...
- :octicons-alert-fill-24:{ .alert-icon } The deprecated `nb_of_workers` attribute of the
  JobConfig has been removed.
- :octicons-alert-fill-24:{ .alert-icon } The deprecated `parent_id` attribute of a DataNode,
  Task, Pipeline, or Scenario entity, has been removed.
- :octicons-alert-fill-24:{ .alert-icon } The deprecated `last_edition_date` and
  `edition_in_progress` attributes of a DataNode entity have been removed.
- :octicons-alert-fill-24:{ .alert-icon } The deprecated `DataNode.lock_edition()` and
  `DataNode.unlock_edition()` methods have been removed.
- :octicons-alert-fill-24:{ .alert-icon } The deprecated `taipy.create_pipeline()` method
  has been removed.
- :octicons-feed-rocket-16:{ .rocket-icon } Function `DataNode.track_edit` has been made public.
- :octicons-bug-24:{ .bug-icon } When running the Orchestrator service in development mode,
...
- :octicons-feed-plus-16:{ .plus-icon } A new template named "scenario-management" is available.
...
- :octicons-feed-rocket-16:{ .rocket-icon } The default template also supports creating a
...
- :octicons-feed-rocket-16:{ .rocket-icon } The "multi-page-gui" template has been removed.
...
- :octicons-feed-plus-16:{ .plus-icon } Python functions including scenario management
...
- :octicons-feed-rocket-16:{ .rocket-icon } The job recovery mechanism is now only
