...
- :octicons-feed-plus-16:{ .plus-icon } A default set of stylesheets are installed with Taipy GUI
  ...
  requirements.
- :octicons-feed-plus-16:{ .plus-icon } The [`table`](../../refmans/gui/viselements/generic/table.md)
...
- :octicons-feed-rocket-16:{ .rocket-icon } The
  ...
  or collapsed.
- :octicons-feed-rocket-16:{ .rocket-icon } Better error messages when parsing Markdown content.
- :octicons-feed-rocket-16:{ .rocket-icon } Better support for auto-completion in IDE for the
  `Gui.run()^` configuration parameters, based on a generated Python Interface Definition file.
- :octicons-feed-rocket-16:{ .rocket-icon } The *status* entry point now provides information
  about the loaded element libraries and the elements they define.
- :octicons-feed-rocket-16:{ .rocket-icon } The `navigate()^` function and the *page* property
...
- :octicons-alert-fill-24:{ .alert-icon } Filters management fixed in the
  [`table` controls](../../refmans/gui/viselements/generic/table.md).<br/>
  See [issue #667](https://github.com/Avaiga/taipy-gui/issues/667).
- :octicons-alert-fill-24:{ .alert-icon } Communication with the server may break.<br/>
