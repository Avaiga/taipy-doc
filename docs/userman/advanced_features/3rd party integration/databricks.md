...
- **Databricks Task**: A Taipy Task that when it is submitted, will trigger the execution of a job in Databrick Workflow.
...
A `DatabricksTableDataNode` data node is a specific data node used to model Databricks table data. To
add a new *Databricks Table* data node configuration, the `Config.configure_databricks_table_data_node()` method
...
In lines 3-9, we configure a basic Databricks Table data node with the id "historical_temperature".
The data node will point to the Databricks table "hist_temp". Its *scope* is by
default `SCENARIO`. The profile to be used is "default" profile with the cluster id is
"0123-456789-dbscluster2". The exposed type of this data node will be the default value "spark".

In lines 11-17, we configure another Databricks Table data node with the identifier "log_history".
