...
- _**storage_folder**_: The folder name used to store user data like data node files.
    Its default value is "user_data/".
- _**taipy_storage_folder**_: The folder name used to store internal Taipy data.
    Its default value is ".taipy/".
...
- _**repository_properties**_: A dictionary of properties that will be used to instantiate the
    ...
    If _**repository_type**_ is set to `filesystem`, Taipy uses _**storage_folder**_ to store
...
In this example, we configure:
  ...
  - Custom values for the *storage_folder*, *taipy_storage_folder*, and *read_entity_retry* parameters.
