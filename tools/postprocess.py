...
"""
    for desc in [
        ...
        ("Templates", "tp_templates/", "tp-content-card--primary", "end-group"),
...
def on_post_build(env):
    ...
    for root, _, file_list in os.walk(site_dir):
        for f in file_list:
            ...
            if f.endswith(".html"):
                filename = os.path.join(root, f).replace("\\", "/")
                ...
                if "rest/apis_" in filename in filename:
                ...
                # Rename the GUI Extension API type aliases
                    ...
                    href_prefix = ""
                    if re.search(r"/(?:functions|interfaces)/.*?/index.html", filename):
                        href_prefix = "../"
                    for in_out in [
                        ...
                        for link in LINK_RE.finditer(html_content):
                            ...
                            new_content += f'<a href="{href_prefix}{in_out[2]}"><code>{in_out[1]}</code></a>'
                ...
                # - Taipy.module.some function -> taipy.module.some_function
                elif "refmans/reference" in filename:
                    ...
                    if re.search(r"/(?!pkg_)\w+/index.html$", filename) and re.search(r"<title>\w+\s+class\s", html_content):
                        # Hack for class 'number': the short doc has a wrong link to the 'input' class
                        if filename.endswith("pkg_builder/number/index.html"):
                            html_content = html_content.replace("<a href=\"input/\">", "<a href=\"../input/\">")
                            file_changed = True
                        # Navigation
                        if m := re.search(
                        ...
                        # Page Builder class?
                        m = re.search(r"/pkg_gui/pkg_builder/(\w+)/index.html$", filename)
                ...
                fn_match = re.search(
                    r"/gui/viselements/(generic|corelements)/(.*?)/index.html",
                    filename,
                )
                ...
                if filename.endswith("refmans/reference/pkg_taipy/index.html"):  # The root 'taipy' package# The root 'taipy' package
                    html_content = re.sub(r"(<h1>)taipy(</h1>)",f"\\1{code('taipy')}\\2", html_content)
                    file_changed = True
                fn_match = re.search(r"refmans/reference/pkg_taipy(\..*)/index.html", filename)
    ...
    # Last minute hacks
    # reference_guiext should have a functions/index.html file
    for f in [f"{site_dir}/refmans/reference_guiext/functions/index.html"]:
        if not os.path.exists(f):
            print(f"CREATING {f}")
            with open(f, "w", encoding="utf-8") as html_file:
