...
#
# The gallery index page is generated from all the folders beneath folders.
# Each folder contains a list of sub folders, each sub folder is a gallery item.
...
# - category: The category of the item (fundamentals, visuals, scenario management, integration or large data)
...
class GalleryStep(SetupStep):
    ...
    def _update_gallery_index_file(self, content):
        with open(self.GALLERY_BASE_PATH + "/index.md_template") as file:
            tpl_content = file.read()
        updated_content = re.sub(r"\[LIST_OF_ITEMS\]", content, tpl_content)
        with open(self.GALLERY_BASE_PATH + "/index.md", "w") as file:
            file.write(updated_content)
